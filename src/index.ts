...
program
  ...
  .option(
    "-o, --output-csv <path>",
    "Path to output CSV. Existing files will be overwritten, missing directories must exist or it will error out at the end.",
    "hashes.csv",
  )
  .action(async (path: string, options: { outputCsv: string }) => {
    await getHashesFromSourcesJSON(path, options.outputCsv);
  });
